...
module ParallelMatrixFormatter
  ...
  #
  # The orchestrator uses a centralized config object for all IPC settings,
  # eliminating direct ENV access and file-based server discovery except
  # for explicitly configured fallback scenarios specified in the config.
  #
  # IPC Configuration Usage:
  # - Uses config['ipc']['server_path'] for server socket/directory path
  # - Uses config['ipc']['prefer_unix_socket'] to determine IPC mode
  # - Uses config['ipc']['server_path_file'] for fallback discovery file
  # - Uses config['ipc']['orchestrator_lock_file'] for process coordination
  # - All paths are pre-computed and validated by ConfigLoader
  #
  class Orchestrator
    ...
    # 
    # This method initializes the IPC server using configuration-provided settings:
    # - Server path and mode are determined by config['ipc'] settings
    # - Fallback discovery mechanisms are enabled only if explicitly configured
    # - ENV variables are set only when config permits fallback discovery
    # - All file paths are generated by ConfigLoader to ensure consistency
    #
    ...
    #
    # This method ensures proper cleanup of all IPC-related resources:
    # - Stops the IPC server and closes all client connections
    # - Removes server path file using config-provided path
    # - Finalizes display output and prints summaries
    def stop
      @running = false
      @ipc_server&.stop
