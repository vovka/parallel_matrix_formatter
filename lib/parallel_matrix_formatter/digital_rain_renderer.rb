  ...
  class DigitalRainRenderer
    ...
    def render_process_column(process_id, progress_percent, column_width = nil)
      ...
      result.chars.each_with_index do |char, i|
        if i >= percent_start && i < percent_end && i - percent_start < percent_str.length
          colored_result += colorize(char, @config["colors"]["percent"])
        else
          colored_result += colorize(char, @config["colors"]["rain"])
        end
      end
    ...
    def render_test_dots(test_results)
      dots = test_results.map do |result|
        case result[:status]
        when :passed
    ...
    def render_matrix_line(time_column, process_columns, test_dots)
      ...
      if test_dots && !test_dots.empty?
        components << test_dots
      end
      components.join(" ")
    end
    ...
    def render_failure_summary(failures)
      ...
      failures.each_with_index do |failure, index|
        ...
        if failure[:location]
          lines << colorize("   Location: #{failure[:location]}", "cyan")
        end
        if failure[:message]
    ...
    def render_final_summary(total_tests, failed_tests, pending_tests, total_duration, process_durations, process_count)
      ...
      summary_parts << colorize("#{failed_tests} failures", "red") if failed_tests > 0
      summary_parts << "#{pending_tests} pending" if pending_tests > 0
